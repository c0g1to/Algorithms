...
public:
    explicit AVLTree(int size = 0) : size(size), root(nullptr) {}
    ...
    void insert(int value) {
        if (search(value, root) == nullptr) {
            if (search_stack.empty())
                ...
                search_stack.top()->children[search_stack.top()->value < value] = new Node(value);
            ++size;
        adjust_by_stack();
        }
        clear_stack();
    ...
    char check(int value) {
        char result = search(value, root) != nullptr ? 'Y' : 'N';
        clear_stack();
        return result;
    }
    ...
    int get_root_balance() {
        return root != nullptr ? root->balance : 0;
    }
...
int main() {
    ...
    char operation;
    io >> N;
    AVLTree avl_set = AVLTree();

    for (int i = 1; i <= N; ++i) {
        io >> operation >> value;
        switch (operation) {
            case 'A':
                avl_set.insert(value);
                io << avl_set.get_root_balance() << '\n';
                break;

            case 'D':
                avl_set.remove(value);
                io << avl_set.get_root_balance() << '\n';
                break;

            case 'C':
                io << avl_set.check(value) << '\n';
                break;
        }
    }
