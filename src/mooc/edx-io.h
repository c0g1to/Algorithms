...
/**
 * Reads the getIndex signed int32. If it fails, exits the code.
 */
int edx_next_i32();

/**
 * Reads the getIndex signed int64. If it fails, exits the code.
 */
long long edx_next_i64();

/**
 * Reads the getIndex double. If it fails, exits the code.
 */
double edx_next_double();

/**
 * Reads at most (max_length - 1) symbols of the getIndex whitespace-free token.
 * Returns the length of the token.
 */
int edx_next_bounded(char *dest, int max_length);

/**
 * Reads the getIndex whitespace-free token.
 */
 ...
 *
 * The token may or may NOT quit with a zero character '\0'. However, the implementation
